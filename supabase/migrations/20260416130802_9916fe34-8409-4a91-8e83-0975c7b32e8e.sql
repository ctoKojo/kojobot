...
-- 1. Prevent saving empty session evaluations (scores = '{}')
CREATE OR REPLACE FUNCTION public.guard_empty_evaluation()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  IF NEW.scores IS NULL OR NEW.scores::text = '{}' OR NEW.scores::text = 'null' THEN
    RAISE EXCEPTION 'Cannot save evaluation with empty scores';
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_guard_empty_evaluation ON session_evaluations;
CREATE TRIGGER trg_guard_empty_evaluation
  BEFORE INSERT OR UPDATE ON session_evaluations
  FOR EACH ROW
  EXECUTE FUNCTION guard_empty_evaluation();

-- 2. Update auto_compute_grades_on_final_exam to filter out empty evaluations
...
BEGIN
  ...
  -- Check if ANY submission needs manual grading — skip auto-compute
  ...
  -- Compute grades — filter out empty evaluations (scores = '{}')
  WITH eval_avgs AS (
  ...
  -- Include students who have exam scores but no evaluations
  all_students AS (
    SELECT student_id FROM eval_avgs
    UNION
  ...
  computed AS (
    SELECT 
      ...
      ROUND(
        (COALESCE(ea.evaluation_avg, 0) * v_eval_weight) + 
        (COALESCE(es.final_exam_score, 0) * v_exam_weight)
      ) as weighted_score
    FROM all_students ast
    LEFT JOIN eval_avgs ea ON ea.student_id = ast.student_id
    LEFT JOIN exam_scores es ON es.student_id = ast.student_id
  )
...
-- 3. Update compute_level_grades_batch to also filter empty evaluations
