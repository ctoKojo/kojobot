...
-- ============================================================
-- curriculum_session_assets: Isolate PDF side effects
-- ============================================================

-- 1. Create table
CREATE TABLE public.curriculum_session_assets (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id UUID NOT NULL UNIQUE REFERENCES public.curriculum_sessions(id) ON DELETE CASCADE,
  student_pdf_path TEXT,
  student_pdf_filename TEXT,
  student_pdf_size INTEGER,
  student_pdf_text TEXT,
  student_pdf_text_updated_at TIMESTAMPTZ,
  processing_status TEXT NOT NULL DEFAULT 'idle' CHECK (processing_status IN ('idle', 'extracting', 'done', 'error')),
...
CREATE TRIGGER update_curriculum_session_assets_updated_at
  BEFORE UPDATE ON public.curriculum_session_assets
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- 3. Enable RLS
ALTER TABLE public.curriculum_session_assets ENABLE ROW LEVEL SECURITY;

-- 4. RLS: Admin full access
CREATE POLICY "admin_full_access_assets" ON public.curriculum_session_assets
  FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'::public.app_role))
  WITH CHECK (public.has_role(auth.uid(), 'admin'::public.app_role));

-- 5. RLS: Students can read assets for sessions they have access to
CREATE POLICY "student_read_assets" ON public.curriculum_session_assets
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1
      FROM public.curriculum_sessions cs
      JOIN public.group_students gs ON gs.is_active = true AND gs.student_id = auth.uid()
      JOIN public.groups g ON g.id = gs.group_id
        AND g.age_group_id = cs.age_group_id
        AND g.level_id = cs.level_id
      WHERE cs.id = curriculum_session_assets.session_id
        AND cs.is_active = true
    )
  );

-- 6. Backfill existing data
INSERT INTO public.curriculum_session_assets (
  session_id, student_pdf_path, student_pdf_filename, student_pdf_size,
  student_pdf_text, student_pdf_text_updated_at,
  processing_status
)
SELECT
  id, student_pdf_path, student_pdf_filename, student_pdf_size,
  student_pdf_text, student_pdf_text_updated_at,
  CASE WHEN student_pdf_text IS NOT NULL THEN 'done' ELSE 'idle' END
FROM public.curriculum_sessions
WHERE student_pdf_path IS NOT NULL OR student_pdf_text IS NOT NULL;

-- 7. RPC: upsert_session_asset (admin only, no optimistic lock)
CREATE OR REPLACE FUNCTION public.upsert_session_asset(
  p_session_id UUID,
  p_student_pdf_path TEXT,
  p_student_pdf_filename TEXT,
  p_student_pdf_size INTEGER
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM curriculum_sessions WHERE id = p_session_id AND is_active = true) THEN
  ...
  INSERT INTO curriculum_session_assets (
    ...
    p_session_id, p_student_pdf_path, p_student_pdf_filename, p_student_pdf_size,
    'extracting'
  )
  ON CONFLICT (session_id) DO UPDATE SET
    student_pdf_path = EXCLUDED.student_pdf_path,
    student_pdf_filename = EXCLUDED.student_pdf_filename,
    student_pdf_size = EXCLUDED.student_pdf_size,
    processing_status = 'extracting',
    student_pdf_text = NULL,
    student_pdf_text_updated_at = NULL,
    last_error_text = NULL
  RETURNING id INTO v_asset_id;

  RETURN jsonb_build_object('id', v_asset_id, 'status', 'extracting');
END;
$$;

-- 8. Update update_curriculum_session: remove PDF fields
...
-- 9. Update get_curriculum_with_access: LEFT JOIN assets
...
AS $function$
  SELECT
    ...
    CASE WHEN has_role(auth.uid(), 'admin'::app_role) THEN csa.student_pdf_path ELSE NULL END,
    CASE WHEN has_role(auth.uid(), 'admin'::app_role) THEN csa.student_pdf_filename ELSE NULL END,
    CASE WHEN has_role(auth.uid(), 'admin'::app_role) THEN csa.student_pdf_size ELSE NULL END,
    (csa.student_pdf_path IS NOT NULL)
  FROM public.curriculum_sessions cs
  LEFT JOIN public.curriculum_session_assets csa ON csa.session_id = cs.id
...
-- 10. Update clone_curriculum: also clone asset rows
...
BEGIN
  ...
  -- Clone sessions (without PDF fields on curriculum_sessions)
  ...
  -- Clone asset rows for sessions that have them
  INSERT INTO curriculum_session_assets (
    session_id, student_pdf_path, student_pdf_filename, student_pdf_size,
...
-- 11. Update storage RLS: student_read_pdf uses assets table
DROP POLICY IF EXISTS "student_read_pdf" ON storage.objects;
...
USING (
  ...
  AND EXISTS (
    SELECT 1
    FROM public.curriculum_session_assets csa
    JOIN public.curriculum_sessions cs ON cs.id = csa.session_id AND cs.is_active = true
    JOIN public.group_students gs ON gs.is_active = true AND gs.student_id = auth.uid()
    JOIN public.groups g ON g.id = gs.group_id
      AND g.age_group_id = cs.age_group_id
      AND g.level_id = cs.level_id
    WHERE csa.student_pdf_path = storage.objects.name
  )
);
