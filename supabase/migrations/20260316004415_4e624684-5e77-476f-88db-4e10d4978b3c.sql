...
BEGIN
  ...
  -- Detect academy_closure cancellation
  v_is_closure_cancel := (NEW.status = 'cancelled' AND NEW.cancellation_reason = 'academy_closure');

  IF NEW.status = 'cancelled' AND NOT v_is_closure_cancel THEN
    -- Non-closure cancellation: increment owed (existing behavior)
    UPDATE groups
  ...
  IF v_is_closure_cancel THEN
    -- Academy closure: replacement keeps SAME session_number
    v_next_session_number := NEW.session_number;
  ELSE
    -- Completed or non-closure cancelled: next number
    v_next_session_number := NEW.session_number + 1;
  END IF;

  -- ======== STOP CONDITION ========
  IF NOT v_is_closure_cancel THEN
    ...
    END IF;
  ELSE
    -- For closure cancels, only stop if session_number exceeds level
    IF v_next_session_number > v_expected_sessions THEN
      RETURN NEW;
    END IF;
  END IF;
