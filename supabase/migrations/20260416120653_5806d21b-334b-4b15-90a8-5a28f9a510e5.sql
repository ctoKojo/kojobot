...
-- Fix the auto_compute_grades trigger to skip when manual grading is needed
CREATE OR REPLACE FUNCTION auto_compute_grades_on_final_exam()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Always update exam_submitted_at for the student who just submitted
  ...
  -- NEW: Check if ANY submission needs manual grading — if so, skip auto-computation
  ...
  IF v_needs_manual > 0 THEN
    -- Just update group level_status to exam_done (all submitted, awaiting manual grading)
...
-- Fix Sayed's data: reset to exam_scheduled with exam_submitted_at
-- First delete the premature level_grades entry
DELETE FROM level_grades 
WHERE student_id = '25d97b93-44bb-4c3e-bcad-fc14fa1b18d0' 
  AND group_id = 'ff6f3d60-3d11-4c1b-a578-ca6ed4f219c3';

-- Reset group_student_progress back to exam_scheduled with exam_submitted_at set
UPDATE group_student_progress 
SET status = 'exam_scheduled',
    outcome = NULL,
    graded_at = NULL,
    exam_submitted_at = '2026-04-16 11:47:12.273+00',
    status_changed_at = now(),
    updated_at = now()
WHERE student_id = '25d97b93-44bb-4c3e-bcad-fc14fa1b18d0'
  AND group_id = 'ff6f3d60-3d11-4c1b-a578-ca6ed4f219c3';

-- Reset group level_status back to exam_done (submitted but needs grading)
UPDATE groups SET level_status = 'exam_done' WHERE id = 'ff6f3d60-3d11-4c1b-a578-ca6ed4f219c3';
