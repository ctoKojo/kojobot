...
-- ============================================================
-- Fix: cancelled sessions should regenerate SAME session_number
-- completed sessions increment to next session_number
-- ============================================================

-- 1. Drop old unique index and recreate excluding cancelled sessions
...
WHERE is_makeup IS NOT TRUE AND session_number IS NOT NULL AND status != 'cancelled';

-- 2. Replace function with corrected logic
...
BEGIN
  ...
  -- Respect level's expected_sessions_count
  ...
  -- Determine next session number based on status:
  -- completed → next number (content delivered, move forward)
  -- cancelled → same number (content NOT delivered, retry)
  ...
  ELSE
    -- cancelled: regenerate same session_number
    v_next_session_number := NEW.session_number;
  END IF;
