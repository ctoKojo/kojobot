...
-- Step 1: Backfill content_number for completed non-makeup sessions
WITH ranked AS (
  SELECT s.id,
         COALESCE(g.starting_session_number, 1) AS start_num,
         ROW_NUMBER() OVER (
...
UPDATE sessions
SET content_number = ranked.start_num - 1 + ranked.rn
FROM ranked
WHERE sessions.id = ranked.id;

-- Step 2: Delete orphan session_number=13 scheduled sessions
...
-- Step 3: Fix owed_sessions_count for groups with academy_closure cancellations
UPDATE groups 
SET owed_sessions_count = GREATEST(COALESCE(owed_sessions_count, 0) - 1, 0)
WHERE id IN (
  SELECT DISTINCT s.group_id 
  FROM sessions s 
  WHERE s.cancellation_reason = 'academy_closure'
)
AND owed_sessions_count > 0;

-- Step 4: Recalculate last_delivered_content_number for all active groups
UPDATE groups g
SET last_delivered_content_number = COALESCE(
  ...
  COALESCE(g.starting_session_number, 1) - 1
)
WHERE g.is_active = true;

-- Step 5: Update trigger - don't increment owed for academy_closure
...
BEGIN
  ...
  IF NEW.status = 'cancelled' AND COALESCE(NEW.cancellation_reason, '') != 'academy_closure' THEN
    UPDATE groups
