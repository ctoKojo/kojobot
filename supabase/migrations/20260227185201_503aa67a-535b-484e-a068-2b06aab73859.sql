...
BEGIN
  ...
  -- ============ Restrict makeup attendance to assigned student ============
  ...
  -- ============ Process each record (INSERT ONLY - no updates) ============
  FOR v_rec IN SELECT * FROM jsonb_array_elements(p_records)
  LOOP
    ...
    -- CHECK FOR DUPLICATE: skip if attendance already exists
    ...
    IF v_status = 'absent' THEN
      ...
      -- INSERT attendance with makeup link (no ON CONFLICT)
    ...
    ELSE
      -- === NOT ABSENT: insert without makeup ===
      INSERT INTO attendance (session_id, student_id, status, notes, recorded_by, compensation_status, makeup_session_id)
