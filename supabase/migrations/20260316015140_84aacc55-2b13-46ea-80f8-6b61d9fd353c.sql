...
-- Update auto_generate_next_session to properly set content_number
...
BEGIN
  ...
  IF v_is_closure_cancel THEN
    -- Academy closure: replacement inherits same content_number
    v_next_content_number := COALESCE(NEW.content_number, NEW.session_number);
  ELSE
    -- Normal completion or non-closure cancel: content advances
