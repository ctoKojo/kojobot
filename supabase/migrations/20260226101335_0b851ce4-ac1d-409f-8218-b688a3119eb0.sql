...
DROP FUNCTION IF EXISTS get_leaderboard(text, uuid, uuid, uuid, uuid, text, int, int);
...
BEGIN
  ...
  ranked AS (
    SELECT
      ...
      COUNT(*) OVER() AS cnt,
      COALESCE(gs.is_active, false) AS gs_is_active
    FROM aggregated a
    ...
    LEFT JOIN group_students gs ON gs.group_id = a.latest_group_id AND gs.student_id = a.student_id
  )
  SELECT
    r.student_id,
    r.full_name,
