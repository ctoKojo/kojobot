...
-- Fix auto_generate_next_session to respect level_status
...
BEGIN
  ...
  -- P0: Stop generation when level is beyond in_progress
  ...
  IF NEW.status = 'cancelled' AND NOT v_is_closure_cancel THEN
    IF v_last_content < v_expected_sessions THEN
      UPDATE groups
...
-- Clean up orphan scheduled/cancelled sessions beyond content 12 for T17
DELETE FROM sessions
WHERE group_id = '05b81214-bbd0-47c0-bde5-8d8a80b699f3'
  AND session_number > 14
  AND status IN ('scheduled', 'cancelled');

-- Reset owed_sessions_count for T17 (was incorrectly incremented)
UPDATE groups
SET owed_sessions_count = 0
WHERE id = '05b81214-bbd0-47c0-bde5-8d8a80b699f3';
