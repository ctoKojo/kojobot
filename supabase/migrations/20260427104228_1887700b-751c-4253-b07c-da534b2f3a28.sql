...
CREATE OR REPLACE FUNCTION public.get_student_level_report(
  p_student_id uuid,
  p_level_id uuid,
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'name', l.name,
    'name_ar', l.name_ar,
    'expected_sessions', l.expected_sessions
  )
  INTO v_level
  FROM levels l
...
REVOKE ALL ON FUNCTION public.get_student_level_report(uuid, uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_student_level_report(uuid, uuid, uuid) TO authenticated;
