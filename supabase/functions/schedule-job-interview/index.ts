...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Build confirm/reschedule link (token was auto-generated by trigger)
    const confirmToken = (interview as any).confirm_token as string
    const siteOrigin = req.headers.get('origin') || 'https://kojobot.com'
    const confirmUrl = `${siteOrigin}/interview/confirm/${confirmToken}`
